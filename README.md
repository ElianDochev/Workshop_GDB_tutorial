...
We have all had the experience of writing a program, running it, and finding out that it segfaults or worse does not work as expected. We all have used printfs on every variable or to display "got to here" to debug our programs. This is a nightmare to do and it wastes a lot of time.
...
Note: many errors can be detected at compile time with the -Wall flag so you may want to add that as well
...
The program will be stuck in an infinite loop. Now we will use gdb to find the bug.
...
Note: if you want you can assign the cmd line arguments in gdb with the following command:
...
info variables				// Vars declared outside the current scope
...
break file::line			// Set breakpoint at a line in a file
...
step 					// Step into function call at the current line (short: s)
finish					// Finish the execution of the current function (short: fin)
...
## If your layout gets broken don't worry just type
...
you will see that the program is stuck on line 57. To demonstrate that we will use the next command
...
Note: you can use a short version of the command by typing **n** instead of **next**

let's print some of the variables to see what is going on
...
let's print some numbers to see what is going on. use **print**. For example print **x**

Hint: Doesn't that "if" statement look suspicious?

## After finding the first bug

Great job! You have found the first bug. Now let's fix it. To do that we must first exit the gdb. To do that we will use the quit command.
I encourage you to think of some other corner cases that our program might run into.
...
Oh no! We have an infinite loop. Let's go back to gdb and see what is going on.

We know how to do that so let's do it.

## Printing arrays

at first glance, it would seem that the problem is in the **is_prime** function again. However, if we step into we will see that it is working as expected (returns 0 on a nonprime). so let's print the array result to see what is going on.
...
Now let's run the program again and see what happens
...
Now we are in the function **getPrimes** the first time it is called, right at the beginning. Let's step into it and examine our variables. remember that we can use **print** to print the value of a variable.
...
Now that we have found the bug with the **x** iterator let's fix it.

Exit gdb.

Remember that we always want 2 as a prime number, so it might be a good idea to start the loop from the next one and just hardcode as the first element of our result array.
...
Another bug, at least it is not an infinite loop anymore, but an incorrect result. Not to worry our trusty friend gdb got our back.

Let's start gdb again and set a breakpoint at the main function. This can be done in two ways:
...
Let's use next to get past our **getPrimes** function
and get to the line **int s = sum(primes, n);**. Now let's print the array result. Remember that we can't just print the array like we did before. we must dereference the pointer to the array first and then specify the number of elements we want to print.
...
As you can see the primes array is correct. Now let's print the sum and see what is going on.
...
Oh no! The sum is wrong. Let's examine the sum function.

## Clearing breakpoints

Let's clear the main breakpoint and set a new one at the sum function.
...
after this let's rerun the program

## Examine the sum function

Using what you have learned so far let's examine the sum function and find the bug.

Hint: what is the initial value of the total? is it **+=** or **=+**?
...
Hmm, this is strange How is the sum of the first 20 primes the same as the sum of the first 10 primes? Let's fire up gdb again and see what is going on.
...
Wow! We see 3 problems. the first **argc** is assigned to 2 not checked if it is 2. The second argv[2] is not initialized and will segfault when referenced(Is that the first argument?). Third, after converting the argument to an int we never assign it to n.

## Fixing the bugs

Let's fix the bugs and recompile the program. Then run it again and see what happens.
...
Oh no! We have a warning. We return a statically declared array from a function. This will kill our program. Let's fix that.
...
Our program is now bug-free. Let's run it one last time and see the result.
...
Well, it seems we still have a bug. Let's go back to gdb and see what is going on.

Use what you have learned so far to find the bug and fix it.

## Congratulations

You have fixed all the bugs and know how to use gdb. You are now a debugging maestro! But are you sure that you have fixed all the bugs? Let's find out.
...
Now let's try to fix **example1.c**

Tired of command line interfaces? Try [GUI in VS code](https://youtu.be/2VokW_Jt0oM?si=YCMhUxXchwhycaUM) 
